...
function humanDelay(text) {
        const base = 800;
        const perChar = 28;
        const length = typeof text === "string" ? text.length : (text?.body?.length || 40);
        const typing = Math.min(length * perChar, 4000);
        const jitter = Math.floor(Math.random() * 1200);
        return base + typing + jitter;
}

async function sendWithHumanBehavior(api, threadID, form, sendFn) {
        try {
                await api.sendTypingIndicator(threadID, true);
        } catch (_) {}
        await new Promise(r => setTimeout(r, humanDelay(typeof form === "string" ? form : form?.body)));
        try {
        ...
        return {
                send: async (form, callback) => {
                        try {
                                ...
                                return await sendWithHumanBehavior(api, event.threadID, form, () =>
                                        api.sendMessage(form, event.threadID, callback)
                                );
                        }
                        catch (err) {
                ...
                reply: async (form, callback) => {
                        try {
                                global.statusAccountBot = 'good';
                                return await sendWithHumanBehavior(api, event.threadID, form, () =>
                                        api.sendMessage(form, event.threadID, callback, event.messageID)
                                );
                        }
                        catch (err) {
                                if (JSON.stringify(err).includes('spam')) {
